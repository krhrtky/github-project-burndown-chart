...
import { Issue, Maybe, ProjectCard, PullRequest, useProjectColumnQuery } from "@/generated/graphql/graphql";
import { getApp } from "@firebase/app";
import { onSnapshot, getFirestore, collection, where, query, documentId } from "@firebase/firestore";
...
type GitHubCard = Maybe<
  { __typename: "ProjectCard" } & Pick<ProjectCard, "id" | "resourcePath" | "databaseId" | "note" | "isArchived"> & {
      content?: Maybe<
        | ({ __typename: "Issue" } & Pick<Issue, "id" | "title">)
        | ({ __typename: "PullRequest" } & Pick<PullRequest, "id" | "title">)
      >;
    }
>;

const title = (card: GitHubCard) => {
...
export const Column: React.FC<Props> = ({ columnId, taskIds }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot<Task>(
      query(collection(getFirestore(getApp()), "task"), where(documentId(), "in", taskIds)),
      (doc) => {
        const result = doc.docs.map((documentSnapshot) => ({
  ...
  const badge = (card: GitHubCard) => {
    switch (card?.content?.__typename) {
      case "Issue":
        return <Badge type="warning">Issue</Badge>;
      case "PullRequest":
        return <Badge style={{ backgroundColor: theme.palette.alert }}>PullRequest</Badge>;
      default:
        return (
          <Badge style={{ backgroundColor: theme.palette.cyanLighter, color: theme.palette.foreground }}>Note</Badge>
        );
    }
  };
  ...
  return (
    <>
      <Grid.Container
        gap={1}
        justify="flex-start"
        style={{ overflowY: "auto", maxHeight: "calc(100vh - (16pt * 11) - 65px)" }}
      >
        {(data?.node?.cards.nodes?.length ?? 0) === 0 ? (
          ...
          data?.node?.cards?.nodes?.map((card) => (
            <Grid xs={12} key={card?.id}>
              ...
              >
                {badge(card)}
                <ReactMarkdown>{title(card) || ""}</ReactMarkdown>
              </Card>
            </Grid>
          ))
        )}
      ...
      {storedTask != null && isFinishedTask(storedTask) ? (
        <FinishedTaskModal open={modalOpening} onClose={closeModal} task={storedTask} />
      ) : null}
      <FinishTaskModal
